...
BEGIN
    -- Clear existing training data first (keep original 20 samples)
    DELETE FROM incident_resolutions WHERE incident_id NOT IN (
        SELECT id FROM incidents ORDER BY created_at LIMIT 20
    );
